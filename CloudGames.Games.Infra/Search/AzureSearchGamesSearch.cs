...
namespace CloudGames.Games.Infra.Search;
...
public class AzureSearchGamesSearch : IGamesSearch
{
    ...
    private static decimal? ToDecimal(SearchDocument doc, string field)
    {
        if (!doc.TryGetValue(field, out var value) || value == null) return null;
    ...
    private static int? ToInt(SearchDocument doc, string field)
    {
        if (!doc.TryGetValue(field, out var value) || value == null) return null;
...
public class NoopGamesSearch : IGamesSearch
{
    ...
    public async Task<IEnumerable<PopularResult>> GetPopularAsync(int top)
    {
        ...
        var counts = events
            .Select(e => SafeExtractGameId(e.Payload))
    ...
    private static Guid SafeExtractGameId(string payload)
    {
        try
        {
            using var doc = System.Text.Json.JsonDocument.Parse(payload);
            if (doc.RootElement.TryGetProperty("GameId", out var gid))
            {
                return gid.GetGuid();
            }
        }
